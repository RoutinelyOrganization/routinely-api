...
export class SessionService {
  ...
  private randomToken = async (
    ...
    const SALT_SESSION = process.env.SALT_SESSION;
    if (!SALT_SESSION) throw new Error('Salt Session not founded');

    if (hasheToken) {
      hashedToken = await hashDataAsync(token, SALT_SESSION);
  ...
  async findExpiredSessionByTokenAndRefreshToken(
    ...
    const hashedRefreshToken = await hashDataAsync(refreshToken, SALT_SESSION);
